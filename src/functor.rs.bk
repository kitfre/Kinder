...
impl<A, B> Functor<A> for Option<B> {
    fn fmap<F>(&self, f: F) -> Option<A> where F: Fn(&B) -> A {
        match *self {
            Some(ref x) => Some(f(x)),
            None => None,
        }
...
impl<A, B> Functor<A> for Box<B> {
    fn fmap<F> (&self, f: F) -> Box<A> where F: Fn(&B) -> A {
        Box::new(f(self))
    }
}
...
impl<A: Ord, B: Ord> Functor<A> for BinaryHeap<B> {
    fn fmap<F>(&self, f: F) -> BinaryHeap<A> where F: Fn(&B) -> A {
...
impl<A: Ord, B:Ord> Functor<A> for BTreeSet<B> {
    fn fmap<F>(&self, f: F) -> BTreeSet<A> where F: Fn(&B) -> A {
...
impl<A: Eq + Hash, B: Eq + Hash> Functor<A> for HashSet<B> {
    fn fmap<F>(&self, f: F) -> HashSet<A> where F: Fn(&B) -> A {
