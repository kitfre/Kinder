...
macro_rules! lift {
    ($t:ident) => {
        impl<A,C> HigherOrder<A> for $t<C> {
            type B = C;
            type C = $t<A>;
        }
    }
...
pub trait Functor<A>: HigherOrder<A> {
...
pub trait Applicative<A> : HigherOrder<A> {
    fn raise(x: A) -> Self::C where Self: HigherOrder<A, B=A>;
    fn apply<F>(&self, <Self as HigherOrder<F>>::C) -> <Self as HigherOrder<A>>::C where F: Fn(&<Self as HigherOrder<A>>::B) -> A, Self: HigherOrder<F>; //kinda ugly
}
...
pub trait Monad<A>: HigherOrder<A> {
    fn lift(x: A) -> Self::C where Self: HigherOrder<A, B = A>;
